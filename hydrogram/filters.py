...
class sudo(Filter):  # noqa: N801
    """Filter messages coming from sudo users (authorized Telegram IDs).

    Sudo users are defined in the Client via the ``sudo_users`` parameter.
    If ``sudo_users`` is empty or not set, ALL messages will be rejected.

    This filter protects your bot from being used by unauthorized users
    when your library/bot is deployed by someone else.
    ...
    Example:
        .. code-block:: python
            ...
            async def handler(client, message):
                await message.reply("You are an authorized sudo user!")
    """
    ...
    async def __call__(self, client, message: Message):
        ...
        return (
            ...
            and message.from_user.id in client.sudo_users
        )


class owner(Filter):  # noqa: N801
    """Filter messages coming from the first/primary owner (first ID in ``sudo_users``).

    The owner is defined as the **first** user ID in the ``sudo_users`` list.
    If ``sudo_users`` is empty, ALL messages will be rejected.
    ...
    Example:
        .. code-block:: python
            ...
            app = Client(
                ...
                api_hash="...",
                bot_token="...",
                sudo_users=[123456789]  # First ID = owner
            )

            @app.on_message(filters.owner)
            async def owner_only(client, message):
                await message.reply("You are the owner!")
    """
    ...
    async def __call__(self, client, message: Message):
        ...
        return (
            message.from_user is not None
            and message.from_user.id == primary_owner
        )
